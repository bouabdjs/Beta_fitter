...
def Predict_fit(a1, b1, a2, b2, a3, b3, W1, W2, epsilon):
    min_x = int(min(a1,a2,a3))
    min_y = int(min(b1,b2,b3))
        
    max_x = int(a1+a2+a3+10)
    max_y= int(b1+b2+b3+10)
    W3 = 1-(W1+W2)
    ...
    Beta3 = stats.beta.pdf(big_grid,a3, b3, loc=0, scale=W3)
    
    stop_limit = Epsilon
    
    conv_pmf1 = signal.fftconvolve(Beta1,Beta2,'full')
    conv_pmf = signal.fftconvolve(conv_pmf1,Beta3,'full')
    ...
    plt.plot(big_grid, Beta3)
    plt.plot(big_grid, conv_pmf)
    string1 = str(W1)+"*Beta(" +str(a1) +"," +str(b1)+")"
    string2 = str(W2)+"*Beta(" +str(a2) +"," +str(b2)+")"
    string3 = str(round(W3, 2))+"*Beta(" +str(a3) +"," +str(b3)+")"
    string4 = "C(" +str(a1) +"," +str(b1)+ ","+ str(a2) +"," +str(b2)+ ","+ str(a3) +"," +str(b3) +"," + str(W1) +"," +str (W2)+")"
    plt.legend([string1, string2, string3, string4], loc="upper right")
    plt.show()
    ...
    for x in range(min_x, 10*(max_x), 1):
        for y in range(min_y, 10*(max_y), 1):
            ...
            if(rms < stop_limit):
                ...
                string3 = "C(" +str(a1) +"," +str(b1)+ ","+ str(a2) +"," +str(b2)+ ","+ str(a3) +"," +str(b3) +"," + str(W1) +"," +str (W2)+")"
...
a_test3 = 0
while a_test3 <= 1:
    a_test3 = float(input("Enter alpha_3 (number greater than 1): "))

b_test3 = 0
while b_test3 <= 1:
    b_test3 = float(input("Enter beta_3 (number greater than 1): "))
    
W1 = 0
while W1 >= 1 or W1 <= 0:
    W1 = float(input("Enter W1 (number less than 1, but greater than 0): "))
        
W2 = 0
while W2 >= 1 or W2 <= 0 or (W1+W2)>=1:
    W2 = float(input("Enter W2 (number less than 1, but greater than 0). Note, W1 + W2 should be less than 1: "))
    
Epsilon = -1
while Epsilon < 0:
    Epsilon = float(input("Enter Epsilon, the minimum limit you accept or 0 for fittest Beta we can find (non-negative number): "))
    
Predict_fit(a_test1, b_test1, a_test2, b_test2, a_test3, b_test3, W1, W2, Epsilon)